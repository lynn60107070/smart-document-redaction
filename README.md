...
End-to-end pipeline to **detect sensitive entities** using **NLP** (spaCy NER plus regex patterns), **map** spans to PDF coordinates where applicable, and **redact** what the user approves. The product is a **Next.js** web app backed by a **FastAPI** server, supporting both **PDF uploads** and **pasted plain text**.
...
The **well-defined problem** this project tackles is: **given document text (from a PDF or from a paste buffer), propose candidate sensitive spans automatically, let a human confirm or correct them, then produce a redacted artifact**—a **downloadable PDF** or **copy-ready text**—suitable for downstream use.
...
| Area | What you get |
|------|----------------|
| **Landing** | Toggle **PDF file** vs **Paste text**; marketing copy describes both paths; **Get started** jumps to the upload card. |
...
| **Paste text** | **Redact text** runs detection + automatic redaction → **redacted output** appears **on the same page** with **Copy** and **Edit**. |
...
(Replace `path/to/smart-document-redaction` with your actual clone path—the directory that contains `backend_api/`, `web/`, and `requirements.txt`.)
